...
* {a, b, c, d, e, f}의 모든 부분집합을 나열하려면<br><pre>
    ...
    3) {b, c, d, e, f}의 모든 부분집합에 {a}를 추가한 집합들을 나열하려면<br>
        ...
        3-3) {c, d, e, f}의 모든 부분집합들에 {a}를 추가한 집합들을 나열하려면<br>
                ...
                3-3-2) {d, e, f}의 모든 부분집합들에 **{a, c}를 추가한 집합들을 나열** 한다.<br></pre>
...
* Java로 구현한 코드에서, <br><pre>
   1) **include** 와 **int k** 는 트리 상에서 현재 위치를 표현.
   2) **if (k == n)**: 만약 내 위치가 리프노드라면,
   3) **include[k] = false; powerSet(k+1);**: 먼저 왼쪽으로 내려갔다가
   4) **include[k] = true; powerSet(k+1);**: 오른쪽으로 내려간다.</pre>
